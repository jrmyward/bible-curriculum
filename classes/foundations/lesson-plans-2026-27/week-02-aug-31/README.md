# Week 2 — Aug 31 – Sep 4, 2026

**Chapter:** Ch 1 — *Introduction: Finding Direction* (Days 6–8 of 8) → **Ch 2 begins Thu**
**Class period:** ~60 min Mon/Tue/Thu · **~45 min Wed** (Wednesday is the short day)
**Sub day:** **Thu Sep 3** (teacher absence — J. Ward, personal) — falls in **Ch 2**, not Ch 1

> Chapter 1 finishes this week on **Wed Sep 2 (the test)**. Thursday starts Chapter 2 —
> those two files are **stubs** until the Ch 2 block is built.

---

## Week Overview

### Daily Plans

- **Mon (Aug 31)** · Day 6 · **Mapmaking** synthesis (Lesson 6) — build a visual map of the Christian worldview
- **Tue (Sep 1)** · Day 7 · Review games + Study Guide (Lessons 8 + 9, merged)
- **Wed (Sep 2)** · Day 8 · **Chapter 1 Test** (`TR 1.10A`, /87) *(short day)*
- **Thu (Sep 3)** · **Ch 2 Day 1** · **SUB DAY** — *stub (Ch 2 block not yet built)*
- **Fri (Sep 4)** · **Ch 2 Day 2** — *stub (Ch 2 block not yet built)*

### Assessments This Week

- **Chapter 1 Test** — Wed Sep 2 (`TR 1.10A`, /87). First Trimester-1 test grade.
- Quiz Cards + Study Guide completion collected Tue (participation).

### Materials Needed (Ch 1 days)

- `PS 1.6A Mapmaking`; poster board, sticky notes, markers per group; scratch paper (Day 6)
- **Three index cards per student**; display `PS 1.8A Chapter 1 Review`; Study Guide SM 21–26 (Day 7)
- `TR 1.10A Chapter 1 Test` — 1 per student + answer key (Day 8)

### Notes

- **Thu Sep 3 is a sub day** — a `substitute-plan-thursday.md` belongs here once Ch 2 is planned.
- Return and review the Ch 1 test early in the Ch 2 block so misconceptions don't harden.
- What the test weights most: **matching the six acts** (Day 3 / Day 7). Aim the review there.

---

## Files in This Directory

- [`1-monday.md`](1-monday.md) — Day 6: Mapmaking synthesis (Lesson 6)
- [`2-tuesday.md`](2-tuesday.md) — Day 7: Review games + Study Guide (Lessons 8 + 9)
- [`3-wednesday.md`](3-wednesday.md) — Day 8: **Chapter 1 Test** *(short day)*
- [`4-thursday.md`](4-thursday.md) — Ch 2 Day 1 · **SUB DAY** *(stub)*
- [`5-friday.md`](5-friday.md) — Ch 2 Day 2 *(stub)*
- [`handouts/`](handouts/) — Week-specific handouts

---

## Post-Week Notes

_Add notes here after teaching this week to inform next year._

### What worked
-

### What needs adjustment
-

### Timing notes
-

### Student feedback
-
