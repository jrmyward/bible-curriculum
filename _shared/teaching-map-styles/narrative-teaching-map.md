...
When building the daily schedule, **mark teacher-absence days with `**SUB:**`** at the start of that day's plan. On a sub day, swap the day's planned narrative-arc activity for a self-contained, non-discussion activity a substitute can facilitate (timeline work, individual map study, journal entry on the previous day's passage, film + reflection, etc.). The `/generate-substitute-plan` skill produces the detailed plan; the teaching map only needs the `**SUB:**` marker.
